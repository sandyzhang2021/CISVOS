...
def get_args():
    parser = argparse.ArgumentParser(description='Eval AFB')
...
def eval_DAVIS(model, model_name, dataloader):
    ...
    for seq_idx, V in enumerate(dataloader):
        ...
        seg_dir = os.path.join('./output33', model_name, seq_name)
        if not os.path.exists(seg_dir):
            os.makedirs(seg_dir)

        if args.viz:
            overlay_dir = os.path.join('./overlay33', model_name, seq_name)
        ...
        for t in tqdm(range(1, frame_n), desc=f'{seq_idx} {seq_name}'):
            ...
            # score, _,npys = model.segment(frames[t:t + 1], fb)
...
def main():
    ...
    if args.level == 1:
        model_name = 'AFB_DAVIS_17val'
        dataset = DAVIS_Test_DS(args.dataset, '2017/val.txt')
        # dataset = DAVIS_Test_DS(args.dataset, '2017/test-dev.txt')
        
       
    elif args.level == 2:
        model_name = 'AFB_YoutubeVOS'
        dataset = YouTube_Test_DS(args.dataset)
    elif args.level == 3:
        model_name = 'AFB_LongVideo'
